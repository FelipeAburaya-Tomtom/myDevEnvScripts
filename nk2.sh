...
#export NAVKIT_REPO_ROOT=$SSD/navkit;
#export NAVCL_REPO_ROOT=$SSD/navcl;
#export VH_REPO_ROOT=$SSD/vhorizon;
export NK2_REPOS_ROOT=$SSD/nk2;
#export IVY_HOME=$SSD/ivy;
...
#export ROUTING_API_AUTH_TOKEN_KEY=AG4j98gQWvNRhb1UtKpXPAQzTARwZkBf
export ANDROID_SDK_ROOT="/usr/local/pkg/android-sdk"

### Locations for some maps
#export NAVKIT_DATASET=/maps/local_maps/navkit;
#export NGL_DATASET=/homes/shared/Data/navkit/AutomaticTests/Data/ngl;
  ...
  then
    ...
    then
      startAppInsideXTerm ENGINE \
        ...
        "../bin/nav-engine --api $ROUTING_API_AUTH_TOKEN_KEY --map $mapDataPath --keystore $keystorePath --password $NDS_DEFAULT_KEYSTORE_PASSWORD --guidance-mode onboard-v2 --mapdisplay-mode onboard 2> engine.log.txt"
    fi
  elif [ "$1" == "online" ];
  then
...
# build NavKit component
changeToBuildDirectory()
{
  if [ -d "$(pwd | grep $NAVKIT_REPO_ROOT)" ];
  then
    cd $NAVKIT_REPO_ROOT/Build;
  elif [ -d "$(pwd | grep $VH_REPO_ROOT)" ];
  then
    cd $VH_REPO_ROOT/Build;
  else
    printf "\n${SetColorToRED}Der derzeitge Pfad gehört weder von NavKit noch von VehicleHorizon!${SetNoColor}\n\n";
  fi
}
buildNavKitWithPkgsrc()
{
  changeToBuildDirectory;
  ./build.py -f Linux-x86_64.yaml --cmake_params IVY_PUBLISH_RESOLVER=publishLocal --cmake_params IVY_FORCE_OVERWRITE=1 "$@";
  local curDirName=$(currentDirectoryName);
  if [ "$curDirName" = "Build" ]; then cd -; fi
}
alias nkbuild=buildNavKitWithPkgsrc;

# build NavCL
buildNavCLWithPkgsrc()
{
  cd $NAVCL_REPO_ROOT/Build;
  python3.7 ./build.py "$@";
  cd -;
}
alias nclbuild=buildNavCLWithPkgsrc;

# find the name of the target for a test
findTestTarget()
{
  nkbuild help | grep "$@" | grep Test | grep '\.ALL:' | awk -F":" '{print $1;}';
}
alias ftt=findTestTarget;

# build a test, set up and run it
buildTestAndRun()
{
  nkbuild "$@";
  ...
  for arg in "$@"
  do
    if [ -n "$(echo $arg | grep '\.ALL$')" ];
    then
      execName=$(echo $arg | sed 's/.ALL//g');
    fi
  done

  local testFilter="$execName.*";
  local isUnitTest=$(echo $execName | grep '_unittest');

  if [ -n "$isUnitTest" ];
  then
    nkbuild "$@" -s Test -tf $testFilter;
  else
    nkbuild "$@" -s Test -tl component,NDS -tf $testFilter;
  fi
}
alias test=buildTestAndRun;

# find the directory where the test has been set up
findTestWorkDir()
{
  local listDirs=$(find . | grep $1 | grep Linux | grep -vi cmake | grep -v '\.xml$\|\.log$\|\.gxl$\|\.py$\|\.pyc$\|\.sqlite$\|/home$\|/Data$');
  local shortListDirs=$(grep --color=always '/test_data\|/work_' <<< "$listDirs");

  if [ -n "$shortListDirs" ];
  then
    printf "$shortListDirs\n";
  else
    printf "$listDirs\n";
  fi
}
alias ftdir=findTestWorkDir;
...
# use the CSV files generated by a run of smoke tests in the
# compare-NDS output folder to update references in the repository
updateRepoSmokeTestsReferences()
{
  for file in $(find . -maxdepth 1 -type f -name "*.csv")
  do
    local newFilePath=$(echo $file \
      | sed 's|_DumpInfo|/DumpInfo|g' \
      | sed 's|_Instructions|/Instructions|g' \
      | sed 's|_Waypoints|/Waypoints|g' \
      | sed 's|_RoadShields|/RoadShields|g' \
      | sed 's|_RoadShieldInfo|/RoadShieldInfo|g' \
      | sed 's|_Routes|/Routes|g' \
      | sed 's|_Lanes|/Lanes|g' \
      | sed 's|_Affixes|/Affixes|g' \
      | sed "s|^\.|$NAVKIT_REPO_ROOT/Engines/Routing/Test/Scripts/etc/InstructionDumper/reference/nds|g");

    mv $file $newFilePath;
  done
}

# decode breakpad for a crash and try to start a debug session with it
decodeBreakpadAndDebugPostMortem()
{
  local expectedDirectory="$NAVKIT_REPO_ROOT/Build/Output/Binary/x86_64-Linux-clang/Debug/bin";
  ls "$expectedDirectory/decode_breakpad_dmp.py" 2&> /dev/null || (printf "\n${SetColorToRED}Zuerst erstellt es das Werkzeug zum Entschlüsseln der Entsorgung...${SetNoColor}\n\n" && nkbuild Framework.ThirdParty.breakpad.Scripts);
  printf "\n${SetColorToRED}Decoding breakpad...${SetNoColor}\n\n";
  local temp="_decoderOutput.temp"
  "$expectedDirectory/decode_breakpad_dmp.py" $1 > $temp;
  mv $1 "$1.decoded";

  local executable=$(grep "MinidumpModule could not determine version for " $temp | head -1 | grep -o '\/.*');
  if [ -n "$executable" ];
  then
    lldb -c "$1.decoded" -f $executable;
  fi

  rm $temp;
}
alias pmdbg=decodeBreakpadAndDebugPostMortem;
...
#checkEnvironmentVariable IVY_HOME          $IVY_HOME;
checkEnvironmentVariable CCACHE_DIR        $CCACHE_DIR;
#checkEnvironmentVariable NAVKIT_REPO_ROOT  $NAVKIT_REPO_ROOT;
checkEnvironmentVariable NK2_REPOS_ROOT    $NK2_REPOS_ROOT;
#checkEnvironmentVariable NAVKIT_DATASET    $NAVKIT_DATASET;
